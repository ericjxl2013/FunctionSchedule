...
public class GetSelectionsName : EditorWindow {
	...
	//递归获取selections名字
	void TransformRecursion(Transform aimTrans, List<string> nameList, List<string> posList, List<string> rotList)
	{
		nameList.Add(aimTrans.name);
		posList.Add(aimTrans.position.x + "," + aimTrans.position.y + "," + aimTrans.position.z);
		rotList.Add(aimTrans.eulerAngles.x + "," + aimTrans.eulerAngles.y + "," + aimTrans.eulerAngles.z);
		if(aimTrans.childCount > 0){
			foreach(Transform child in aimTrans){
				TransformRecursion(child, nameList, posList, rotList);
			}
		}
	}
}
